...
EPS_DECAY = 5000  # Rate at which epsilon decreases
LR = 1e-4  # Learning rate for the optimizer
TARGET_UPDATE = 250  # How often to update the target network
MEMORY_CAPACITY = 10000  # Capacity of the replay memory
NUM_EPISODES = 25000  # Number of episodes to train
...
class DQN_linear(nn.Module):
    def __init__(self, input_dim, output_dim):
        ...
        self.head = nn.Linear(32, output_dim)  # Output layer to produce the final outputs
...
policy_net = DQN_linear(8,env.action_space.n).to(device)  # Initialize policy network for action value estimation
target_net = DQN_linear(8,env.action_space.n).to(device)  # Initialize target network to stabilize learning
...
def select_action(state, steps_done):
    ...
    if sample > eps_threshold:
        ...
        with torch.no_grad():  # Temporarily set all the requires_grad flag to false
            ...
            # print(state.shape)
            # print("NET RETURNS: ")
            # print(policy_net(state).argmax().unsqueeze(0).unsqueeze(0))
            return policy_net(state).argmax().unsqueeze(0).unsqueeze(0)  # Get the action with the highest Q-value 
    else:
        # print("RANDOM ACTION")
...
def log_to_file(log_file, episode, total_reward, average_loss, epsilon):
    with open(log_file, 'a') as f:
        f.write(f"{episode},{total_reward},{average_loss},{epsilon}\n")

# initialize log file
log_file_path = os.path.join(checkpoint_dir, 'training_log.csv')
# Initialize log file and write headers
with open(log_file_path, 'w') as log_file:
    log_file.write("Episode,Total Reward,Average Loss,Epsilon\n")
...
for i_episode in range(NUM_EPISODES):  # Loop over each episode.
    ...
    for t in count():  # Infinite loop for each time step in the episode.
        ...
        if not done:
            ...
            next_state = torch.tensor(next_state).float().to(device).unsqueeze(0)
        else:
            next_state = None
        ...
        loss = optimize_model()  # Optimize the model using the collected experience.
        if loss is not None:
            losses.append(loss)
        # if t % 100 == 0:
    ...
    if losses:
        average_loss = sum(losses) / len(losses)  # Calculate the average loss for the episode
    else:
        average_loss = 0  # No losses to average

    epsilon_value = EPS_END + (EPS_START - EPS_END) * math.exp(-1. * (i_episode) / EPS_DECAY)

    # Logging the results to file after each episode
    log_to_file(log_file_path, i_episode, total_reward, average_loss, epsilon_value)

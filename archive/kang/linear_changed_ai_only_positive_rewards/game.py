...
class AirHockeyGame:
    ...
    def update_pve(self):
        if self.basic_ai_paddle:
            ...
            if self.puck.y > self.basic_ai_paddle.y + self.basic_ai_difficulty * 150: # creates slight reaction delay
                self.basic_ai_paddle.dy = self.basic_ai_paddle.speed
            elif self.puck.y < self.basic_ai_paddle.y - self.basic_ai_difficulty * 150: # creates slight reaction delay
                self.basic_ai_paddle.dy = -self.basic_ai_paddle.speed
            else:
                self.basic_ai_paddle.dy = 0
            if self.puck.x > self.basic_ai_paddle.x + self.basic_ai_difficulty * 75: # creates slight reaction delay
                self.basic_ai_paddle.dx = self.basic_ai_paddle.speed
            elif self.puck.x < self.basic_ai_paddle.x - self.basic_ai_difficulty * 75: # creates slight reaction delay
                self.basic_ai_paddle.dx = -self.basic_ai_paddle.speed
            else:
    ...
    def calculate_rewards(self):
        ...
        if self.game_over:
            if self.winner == "Player 2":
                reward += 20  # reward for winning
        # puck collision with paddles
        if self.rl_ai_paddle and self.check_paddle_collision(self.rl_ai_paddle):
            reward += 5  # Reward for paddle2 (rl_ai_paddle) coming into contact with the puck
        return reward  
    ...
    # FOR NUMERIC ARRAY
    def get_state(self):
        # Return player paddle, puck, AI paddle position, and puck velocity as a numpy array
        array_state = np.array([self.player1_paddle.x, self.player1_paddle.y, self.puck.x, self.puck.y, self.player2_paddle.x, self.player2_paddle.y, self.puck.dx, self.puck.dy])
        # RETURNS A 1D ARRAY with shape of (8,)
        return array_state

...
class Puck:
    ...
    def check_timeout(self, current_time):
        ...
        if self.last_hit_time and (current_time - self.last_hit_time > 10):
    ...
    def move(self, dt):
        ...
        self.x += self.dx * dt
        self.y += self.dy * dt
        friction = 0.99999999  # Apply a slight friction to slow the puck over time
        self.dx *= friction
        self.dy *= friction

        # Enforce a maximum speed to prevent the puck from moving too fast
        speed = math.sqrt(self.dx**2 + self.dy**2)
        max_speed = 800  # Define the maximum allowable speed
        if speed > max_speed:
